"""
Visualization for Translation Decoupled MeanFlow (TDMF) model in 2D
"""
import numpy as np
...
def visualize_tdmf_2d(trajectories, mean_onestep, z_samples, x_data, save_path=None, epoch=None, loss_logs=None, cfm_type=None):
    """
    Visualize Translation Decoupled MeanFlow (TDMF) model in 2D with trajectories
    ...
    Args:
        ...
        loss_logs: Dictionary with loss information {'struct', 'trans', 'lambda'} (optional)
    ...
    # Create title with loss info
    cfm_str = f' ({cfm_type.upper()})' if cfm_type else ''
    if epoch is not None and loss_logs is not None:
        title = (f'TDMF{cfm_str} (Epoch {epoch})\n'
                f'struct={loss_logs["struct"]:.4f}, trans={loss_logs["trans"]:.4f}, '
                f'λ={loss_logs["lambda"]:.3f}')
    elif epoch is not None:
        title = f'Translation Decoupled MeanFlow{cfm_str} (Epoch {epoch})'
    else:
        title = f'Translation Decoupled MeanFlow{cfm_str} (TDMF)'
    ...
    if save_path:
        ...
        if epoch is not None:
            ...
            print(f"TDMF 2D visualization saved to {save_path}")
    
    return fig, ax


if __name__ == '__main__':
    print("Testing TDMF 2D visualization...")
    ...
    visualize_tdmf_2d(
        ...
        z_samples=z_init,
        x_data=x_data,
        save_path='/home/user/Desktop/Gen_Study/outputs/test_viz_tdmf_2d.png',
        epoch=1,
        loss_logs={'struct': 0.1234, 'trans': 0.0567, 'lambda': 0.1}
    )
    print("Test complete!")
