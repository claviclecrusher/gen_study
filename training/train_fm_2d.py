"""
Training script for 2D Flow Matching model
"""
import torch
...
def train_fm_2d(n_samples=500, epochs=2000, lr=1e-3, batch_size=64, seed=42, device='cpu',
                viz_interval=None, viz_output_dir=None, n_infer=200):
    """
    Train 2D Flow Matching model
    ...
    model = FlowMatching(input_dim=2).to(device)
    ...
    for epoch in range(epochs):
        ...
        for i in range(n_batches):
            ...
            z_batch = torch.randn(batch_size_actual, 2).to(device)
        ...
        if viz_interval is not None and viz_output_dir is not None:
            if (epoch + 1) % viz_interval == 0:
                ...
                viz_path = os.path.join(viz_output_dir, f'fm_2d_epoch{epoch+1:04d}.png')
                visualize_fm_2d(
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Train 2D Flow Matching model')
    ...
    save_path = os.path.join(args.output_dir, 'fm_2d_model.pt')
    ...
    viz_path = os.path.join(args.output_dir, 'fm_2d_visualization.png')
    visualize_fm_2d(
        z_samples=z_train,
        x_data=x_train,
        trajectories=trajectories,
